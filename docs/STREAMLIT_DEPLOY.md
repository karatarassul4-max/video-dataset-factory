...
- `upload videos`: accepts up to 10 short clips for an interactive CPU demo.
...
2. Use `use demo data` so viewers immediately see metrics.
3. Switch to `upload videos` and upload 2-5 short clips.
4. For larger datasets, upload `manifest_deduped.jsonl` through `upload manifest JSONL`.
5. Download `manifest.jsonl` and `dataset_summary.md` from the Exports section.
